...
func (s *service) saveTokenToCache(subject *Subject, val string) (err error) {
	if err = ca.Set(tokenIDKeyPrefix+subject.ID, val, tokenExpiredDate).Err(); err != nil {
		return fmt.Errorf("[saveTokenToCache]: token保存到redis发生错误，err: %v", err.Error())
	}
	return
}

func (s *service) getTokenFromCache(subject *Subject) (token string, err error) {
	tokenCache, err := ca.Get(tokenIDKeyPrefix + subject.ID).Result()
	if err != nil {
		return token, fmt.Errorf("[getTokenFromCache] 获取token发生错误，err: %v", err.Error())
	}
	return string(tokenCache), nil
}

func (s *service) deleteTokenFromCache(subject *Subject) (err error) {
	if err = ca.Del(tokenIDKeyPrefix + subject.ID).Err(); err != nil {
		return fmt.Errorf("[deleteTokenFromCache] 清理Token缓存发生错误， err: %v", err.Error())
	}
	return
}
