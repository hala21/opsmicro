...
var (
	...
	tokenExpiredTopic = "demo.micro.book.topic.auth.tokenExpired"
)
...
// standardClaims token 标准stardard claims
...
func (s *service) MakeAccessToken(subject *Subject) (ret string, err error) {
	...
	if err != nil {
		return "", fmt.Errorf("[MakeAccessKey] 创建访问Key失败，err： %s", err)
	}
	err = s.saveTokenToCache(subject, ret)
	if err != nil {
		return "", fmt.Errorf("[MakeAccessKey] save Key失败，err： %s", err)
	}
...
func (s *service) DelUserAccessToken(tk string) (err error) {
	...
	// 通过解析到的用户id删除
	err = s.deleteTokenFromCache(&Subject{
		ID: claims.Subject,
	})
